"""ベクトルストアの構築および文書の追加に関する関数を定義するモジュール。

このモジュールでは、PDF ファイルから文書を読み込み、ベクトルストアに追加する処理や、
クエリに対する文脈の構築を行います。
"""
...
def build_context(vectorstore: VectorStore, query: str) -> str:
    ...
    contexts = [
        "\n".join([f"page_content: {page.page_content}", f"metadata: {page.metadata}"])
        for page in pages
    ]
