"""RAG（Retrieval-Augmented Generation）に関連するクラスと関数を定義するモジュール。

このモジュールでは、ベクトルストアを用いた文脈構築とチャットモデルによる
質問応答システムの実装を行います。
"""
...
@dataclass
class RAG(ABC):
    """RAG（Retrieval-Augmented Generation）の基底クラス。

    Attributes:
        vectorstore (VectorStore): 使用するベクトルストア。
        chat_model_option (ChatModelOption): 使用するチャットモデルのオプション（デフォルトは AZURE）。
    """
...
class ResearchAssistant(RAG):
    """ResearchAssistant クラスは、質問に対する文脈に基づいた回答を生成する RAG の実装です。
    ...
    def output_structure(self) -> BaseModel:
        ...
        Returns:
            BaseModel: ResearchAssistantResponse モデル。
        """
